...
- `stream_animation` - Reveal streamed assistant text, reasoning, and tool output progressively instead of appearing in bursts. Defaults to `false`.
- `stream_animation_cps` - Reveal rate in characters per second when `stream_animation` is enabled. Defaults to `20`. Adjustable at runtime from the command palette ("Slower/Faster stream animation").
